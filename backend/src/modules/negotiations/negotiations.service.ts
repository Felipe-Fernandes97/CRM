...
export class NegotiationsService {
  ...
  async create(createDto: CreateNegotiationDto): Promise<Negotiation> {
    const negotiation = this.negotiationsRepository.create(createDto);
    return this.negotiationsRepository.save(negotiation);
  }

  async findAll(paginationDto: PaginationDto): Promise<PaginatedResult<Negotiation>> {
    const { page = 1, limit = 10, search, sortBy = 'criadoEm', sortOrder = 'DESC' } = paginationDto;

    const queryBuilder = this.negotiationsRepository.createQueryBuilder('negotiation');
  ...
  async update(id: string, updateDto: UpdateNegotiationDto): Promise<Negotiation> {
